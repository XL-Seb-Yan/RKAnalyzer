...
void FillHistogram(TH1F* hist1_1, TH1F* hist1_2, TH1F* hist1_3, TH1F* hist1_4, TH1F* hist1_5, TH1F* hist1_6, TH1F* hist1_7, TH1F* hist1_8, TH1F* hist1_9, TH1F* hist1_10,
    ...
    // outputTree->Branch("BCand_ID_l1", &output_BCand_ID_l1, "BCand_ID_l1/F");
    // outputTree->Branch("BCand_ID_l2", &output_BCand_ID_l2, "BCand_ID_l2/F");
        ...
        tree->GetEntry(ievt);
				...
				for(size_t iB = 0; iB < BToKEE_mass.GetSize(); ++iB) {
					...
					for(int i=0; i<genHad_p4s.size();i++){
						if(kp4.DeltaR(genHad_p4s[i]) < 0.03) {
					...
					if(matchingbit_trk[0]) isMatched_trk = true;
			...
			counter3++;
			int l1_index = BToKEE_l1Idx[iB_matched];
			int l2_index = BToKEE_l2Idx[iB_matched];
			int k_index = BToKEE_kIdx[iB_matched];
			if(BToKEE_fit_mass[iB_matched] < 4.7 || BToKEE_fit_mass[iB_matched] > 5.7) continue;
			if(BToKEE_mll_fullfit[iB_matched]*BToKEE_mll_fullfit[iB_matched] < 1.1 || BToKEE_mll_fullfit[iB_matched]*BToKEE_mll_fullfit[iB_matched] > 6.0) continue;		
			...
			if(!Electron_isPF[l1_index] || !Electron_isPF[l2_index] || Electron_PFEleMvaID_RetrainedRawValue[l1_index] < 0 || Electron_PFEleMvaID_RetrainedRawValue[l2_index] < 0 || BToKEE_mll_charge[iB_matched] != 0) {continue;}
			counter4++;
			
			
			TLorentzVector l1p4, l2p4, Kp4, Bp4;
			l1p4.SetPtEtaPhiM(BToKEE_fit_l1_pt[iB_matched], BToKEE_fit_l1_eta[iB_matched], BToKEE_fit_l1_phi[iB_matched], 0.000511);
			l2p4.SetPtEtaPhiM(BToKEE_fit_l2_pt[iB_matched], BToKEE_fit_l2_eta[iB_matched], BToKEE_fit_l2_phi[iB_matched], 0.000511);
			Kp4.SetPtEtaPhiM(BToKEE_fit_k_pt[iB_matched], BToKEE_fit_k_eta[iB_matched], BToKEE_fit_k_phi[iB_matched], 0.493677);
			Bp4.SetPtEtaPhiM(BToKEE_fit_pt[iB_matched],BToKEE_fit_eta[iB_matched],BToKEE_fit_phi[iB_matched],BToKEE_fit_mass[iB_matched]);
			TLorentzVector llp4;
			llp4 = l1p4 + l2p4;
			...
			ROOT::Math::XYZVector BVtxVector(BToKEE_vtx_x[iB_matched] - PV_x[0], BToKEE_vtx_y[iB_matched] - PV_y[0], BToKEE_vtx_z[iB_matched] - PV_z[0]);
			ROOT::Math::XYZVector unitBVtxVector = BVtxVector / BVtxVector.R();
			
			hist1_1->Fill(BToKEE_fit_mass[iB_matched]); 
			hist1_2->Fill(BToKEE_mll_fullfit[iB_matched]); output_BCand_mll_fullfit_norm = BToKEE_mll_fullfit[iB_matched] / BToKEE_fit_mass[iB_matched];
			hist1_3->Fill(Electron_pt[l1_index]);
			hist1_4->Fill(Electron_pt[l2_index]);
			hist1_5->Fill(Electron_pt[l1_index] / BToKEE_fit_mass[iB_matched]); output_BCand_fit_l1_pt_norm = Electron_pt[l1_index] / BToKEE_fit_mass[iB_matched];
			hist1_6->Fill(Electron_pt[l2_index] / BToKEE_fit_mass[iB_matched]); output_BCand_fit_l2_pt_norm = Electron_pt[l2_index] / BToKEE_fit_mass[iB_matched];
			hist1_7->Fill(ProbeTracks_pt[k_index]); 
			hist1_8->Fill(0);
			hist1_9->Fill(ProbeTracks_pt[k_index] / BToKEE_fit_mass[iB_matched]); output_BCand_fit_trk1_pt = ProbeTracks_pt[k_index] / BToKEE_fit_mass[iB_matched];
			hist1_10->Fill(0);                                               output_BCand_fit_trk2_pt = 0;
			hist1_11->Fill(BToKEE_fit_pt[iB_matched]);
			hist1_12->Fill(BToKEE_fit_pt[iB_matched] / BToKEE_fit_mass[iB_matched]); output_BCand_fit_pt_norm = BToKEE_fit_pt[iB_matched] / BToKEE_fit_mass[iB_matched];
			...
			hist1_14->Fill(BToKEE_svprob[iB_matched]); output_BCand_svprob = BToKEE_svprob[iB_matched];
			hist1_15->Fill(BToKEE_l_xy[iB_matched] / BToKEE_l_xy_unc[iB_matched]); output_BCand_sigLxy = BToKEE_l_xy[iB_matched] / BToKEE_l_xy_unc[iB_matched];
			hist1_16->Fill(l1p4.DeltaR(l2p4)); output_BCand_dR_ll = l1p4.DeltaR(l2p4);
			output_BCand_MomenAsym_lltrk1 = ((llp3Vector.Cross(unitBVtxVector)).R() - (Kp3Vector.Cross(unitBVtxVector)).R()) / ((llp3Vector.Cross(unitBVtxVector)).R() + (Kp3Vector.Cross(unitBVtxVector)).R());
			hist1_17->Fill(output_BCand_MomenAsym_lltrk1);
			output_BCand_MomenAsym_lltrk2 = 1;
			hist1_18->Fill(output_BCand_MomenAsym_lltrk2);
			output_BCand_MomenAsym_llKs = 1;
			hist1_19->Fill(output_BCand_MomenAsym_llKs);
			hist1_20->Fill(BToKEE_l1_iso04[iB_matched]); output_BCand_iso04_l1 = BToKEE_l1_iso04[iB_matched];
			hist1_21->Fill(BToKEE_l2_iso04[iB_matched]); output_BCand_iso04_l2 = BToKEE_l2_iso04[iB_matched];
			hist1_22->Fill(BToKEE_k_iso04[iB_matched]); output_BCand_iso04_trk1 = BToKEE_k_iso04[iB_matched];
			hist1_23->Fill(-1); output_BCand_iso04_trk2 = -1;
			hist1_24->Fill(BToKEE_k_svip3d[iB_matched]/BToKEE_k_svip3d_err[iB_matched]); output_BCand_sigImpact_lltrk1 = BToKEE_k_svip3d[iB_matched]/BToKEE_k_svip3d_err[iB_matched];
			hist1_25->Fill(-1); output_BCand_sigImpact_lltrk2 = -1;
			output_BCand_dR_l1trk1 = l1p4.DeltaR(Kp4);
			hist1_26->Fill(output_BCand_dR_l1trk1);
			output_BCand_dR_l1trk2 = -1;
			hist1_27->Fill(output_BCand_dR_l1trk2);
			output_BCand_dR_l2trk1 = l2p4.DeltaR(Kp4);
			hist1_28->Fill(output_BCand_dR_l2trk1);
			output_BCand_dR_l2trk2 = -1;
			hist1_29->Fill(output_BCand_dR_l2trk2);
			output_BCand_dR_trk1trk2 = -1;
			hist1_30->Fill(output_BCand_dR_trk1trk2);
			hist1_31->Fill(BToKEE_fit_cos2D[iB_matched]); output_BCand_fit_cos2D = BToKEE_fit_cos2D[iB_matched];
			output_BCand_dz_l1trk1 = Electron_vz[l1_index] - ProbeTracks_vz[k_index];
			hist1_32->Fill(output_BCand_dz_l1trk1);
			output_BCand_dz_l1trk2 = -1;
			hist1_33->Fill(output_BCand_dz_l1trk2);
			output_BCand_dz_l2trk1 = Electron_vz[l2_index] - ProbeTracks_vz[k_index];
			hist1_34->Fill(output_BCand_dz_l2trk1);
			output_BCand_dz_l2trk2 = -1;
			hist1_35->Fill(output_BCand_dz_l2trk2);
			output_BCand_dz_trk1trk2 = -1;
...
void analyzer_MC_131_triplets(TString filename, int ifile, int mode) {
